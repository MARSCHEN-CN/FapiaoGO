...
# ── 锚点关键词 ──
# 注意:同时收录"购货方/销货方"写法(部分发票使用此说法,如广东电网等)。
# 顺序:长词优先,避免短词先命中导致定位偏移。
BUYER_ANCHORS: tuple = (
    '购买方信息', '购货方信息', '购方信息',
    '购买方', '购货方',
)
...
COMPANY_SUFFIX_LIST: tuple = (
    ...
    '集团', '厂', '店', '中心', '事务所', '工作室', '合伙企业',
    '个人独资企业', '合作社', '部', '行', '室', '处', '协会', '商会',
    '学校', '医院', '银行', '研究院', '基金会',
    '加油站', '加气站', '充电站',
    # 以下为 2026-08-31 取证补齐：非公司制但可开具正规发票的经营主体后缀
    # 仅加入实测证明缺失、且不与已有子串重复的项（饭店/酒店/药店等靠「店」覆盖，不加）
    '餐厅', '酒楼', '宾馆', '咖啡馆', '茶楼',
    '诊所', '药房',
    '美容院', '养生馆',
    '驾校', '幼儿园',
