...
test('FILE_ALERT_PRIORITY 顺序: failed > previousYear > duplicate', () => {
  assert.deepEqual(FILE_ALERT_PRIORITY, ['failed', 'previousYear', 'duplicate'])
})
...
test('resolveStatsMode: 无失败时 previousYear 优先于 duplicate', () => {
  assert.equal(resolveStatsMode({ hasFailed: false, previousYearCount: 2, duplicateCount: 5 }), 'previousYear')
})
test('resolveStatsMode: 仅有 duplicate 时返回 duplicate', () => {
  assert.equal(resolveStatsMode({ hasFailed: false, previousYearCount: 0, duplicateCount: 5 }), 'duplicate')
})
...
test('applySort: 分区顺序 失败 > 往年 > 重复 > 正常（置顶）', () => {
  const files = [
  ...
  // 失败在最前，往年次之，重复组再次，正常最后；段内按 fileName 升序
  // 正常段 n1(n-zzz) 与 n2(n-yyy) 按名称升序 → n2 在前
  assert.deepEqual(keys, ['f1', 'py1', 'py2', 'd1a', 'd1b', 'n2', 'n1'])
})

test('applySort: 往年+重复 同体文件归入「往年」段（优先于重复段）', () => {
  const files = [
  ...
  const sorted = applySort(files, 'fileName', 'asc', duplicateInfo, previousYearInfo)
  const keys = sorted.map(f => f.key)
  // py_dup 同时满足往年与重复，按优先级落在往年段（最前），重复段 d2 随后，正常 n1 最后
