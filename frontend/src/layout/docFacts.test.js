...
test('无记录 → 推导天然方向，isAuto=true，shouldPersist=true（写回供会话内复用）', () => {
  const r = computeInitialDocFacts(null, 'landscape')
  assert.equal(r.requestedPaperOrientation, 'landscape')
...
test('无记录且天然方向为 null → 回落 portrait', () => {
  ...
  assert.equal(r.requestedPaperOrientation, 'portrait')
  assert.equal(r.isAuto, true)
})

test('有记录(portrait) → 返回记录，isAuto=false', () => {
  const r = computeInitialDocFacts({ requestedPaperOrientation: 'portrait', contentRotation: 0 }, 'landscape')
  assert.equal(r.requestedPaperOrientation, 'portrait')
  assert.equal(r.contentRotation, 0)
...
test('有记录(landscape) + contentRotation=90 → 原样返回', () => {
  const r = computeInitialDocFacts({ requestedPaperOrientation: 'landscape', contentRotation: 90 }, 'portrait')
  assert.equal(r.requestedPaperOrientation, 'landscape')
  assert.equal(r.contentRotation, 90)
  assert.equal(r.isAuto, false)
})

test('记录 requestedPaperOrientation 非法 → 视为无记录，回落天然方向', () => {
  const r = computeInitialDocFacts({ requestedPaperOrientation: 'auto' }, 'landscape')
  assert.equal(r.requestedPaperOrientation, 'landscape')
  assert.equal(r.isAuto, true)
  assert.equal(r.shouldPersist, true)
})
