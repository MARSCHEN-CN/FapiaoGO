...
export default memo(function PreviewCanvas({ previewFile, displayInfo, previewCanvas, grayscale }) {
  ...
  //    仅当同一 DOM canvas + 同 source canvas + 同滤镜时跳过重绘
  ...
  const canvasCallbackRef = useCallback((node) => {
    ...
    // L1 命中：同一 DOM 节点 + 同 source canvas + 同滤镜 → 内容还在，跳过
    if (lastNodeRef.current === node && lastSourceRef.current === previewCanvas && lastGrayscaleRef.current === grayscale) return
