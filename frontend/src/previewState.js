...
// ── PaperLayout 构造计数（v1.1 F3 单一构造点守卫）──
// 仅 computePaperLayout（唯一工厂）会使其自增。语义（2026-07-14 重定义）：
//   count = PaperLayout「生成次数」= 对 computePaperLayout 的调用次数。
//   此前（useEffect 时代）误等于「recompute 次数」；改为 useMemo 派生后，
//   同语义仍为「生成次数」，与渲染时机（effect / render）解耦。
//   • mount 后 count=1；
//   • 连续修改 margin/纸张 → count 随每次 relayout +1（relayout 非 reload）；
//   • 切换 100 个文件 → count 不变（deps 不含 previewFile，memo 不重算），证明已与 DocumentState 解耦。
...
export function computePaperLayout(spec) {
  ...
  return {
    ...
    // slotMarginPx：每张虚拟纸（slot）的内部 margin（px，对应 5mm）。
    //
    // Virtual Paper Geometry 两层语义：
    //   Layer 1: paper → usableRect  （outer margin = pageMargin）
    //   Layer 2: slot  → contentRect（slot margin = 5mm，四周均匀内缩）
    //
    // SlotLayout 消费此值对每个 slot 做内缩 → contentRect，使 V16 slot 几何与
    // createLayout（ComposeSlotLayoutFactory）的 slot 内缩行为一致。
    slotMarginPx: Math.round(5 / 25.4 * dpi),
