...
- 集成缺口(真实工作量,非新引擎)：G1a 纯 source(`main.js:605` else + print-settings.js:188 默认 contain)不调引擎→Sumatra fit；G1b `main.js:583` 漏传 `opts.paperW_mm/H_mm`→add-pdf-margins.py 兜底源 MediaBox（Gate2 case 源=目标巧合正确）；G1c `pdfMargin.process` 无 content_rotation 槽位→旋转断链；**G1d 无离散 Translator 层**（§9.4 设计未落地；bake 路径隐式靠 buildPrintExecutionPlan 一次性算 contentRotation+native paper，source/margin 路径完全缺失，R6 防护无从落地）。引擎身份已确认：add-pdf-margins.py L51 `from margin_contract import apply_pdf`，golden 测的是真引擎。未改生产代码。
