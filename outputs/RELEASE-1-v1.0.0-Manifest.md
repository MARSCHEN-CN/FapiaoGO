# FapiaoGO v1.0.0 Release Manifest（RELEASE-1 · RC v4）

- 发布日期：2026-08-31（RC v4 = RC v3 + SIZE-2-A pdf_tool 排除 numpy）
- Release Source（冻结）：**`d7ecadf`**（已 push 远端，经 `git ls-remote` 权威确认）
...
## 交付物（release_final_v4/，两件套：1 个安装程序 + 1 个绿色版压缩包）

| 文件 | 大小 | SHA-256 |
|---|---:|---|
| `FapiaoGO-Setup-1.0.0.exe`（安装程序，未签名） | 234.1 MB | `ac02b48c7943f65c49064ae9b577364197ec3b08b5e7fd28b9a30defdd348e4f` |
| `FapiaoGO-v1.0.0-Windows-x64-Portable.zip` | 300.3 MB | `40f0e4ff4fe5959a0ece0ecc7b34460c7cec804884b5c162e6dba7044f119b81` |

（机器可读：`outputs/SHA256SUMS-v1.0.0.txt`，已覆盖为 v4 值；Release 附件还需 `latest.yml`）
...
- 旧 `release_final/`（v1）、`release_final_v2/`（RC v2）、`release_final_v3/`（RC v3）产物作废，勿用于发布

## Gate 状态（RELEASE-1 · RC v4）

| Gate | 内容 | 状态 |
|---|---|---|
| R0 | Source Freeze：`d7ecadf`，SIZE-2-A 独立提交且已 push（远端经 ls-remote 确认） | ✅ PASS |
| R1 | Clean Build（前端 vite 重建 6.09s，V4 FIX 编译进 dist/app.asar） | ✅ PASS |
| R2 | 后端重建：server.exe（含 party 修复，PYZ 实证 `party_constants`）；**pdf_tool 重建（排除 numpy）** | ✅ PASS |
...
| R5 | Runtime Smoke：server /health=ok；真实发票 OCR amount=133.08 failed_fields=[]；图片 OCR 139.99；**pdf_tool 三条冒烟（margin/placement-bake/png-to-pdf）在 dist 版与 prod 版各跑一遍全 PASS** | ✅ PASS（沙箱可做部分） |
| R6 | Core Regression（Display / Print Preview / 实际打印 / Merge-batch） | ⏳ 真机验收（见清单） |
| R7 | Portable.zip 单件压缩（两件套标准，Installer.zip 废止） | ✅ PASS |
| R8 | SHA-256 + latest.yml（size 245490889 与 Setup.exe 字节数精确匹配）+ Manifest | ✅ PASS |
| R9 | Release Freeze | ⏳ 待真机 R6 通过后定 |

## SIZE-2 验证矩阵（RC v4 实测）
...
- ✅ **push 已完成**：远端 `rotation-b1-hardening` = `d7ecadf`（`git ls-remote origin rotation-b1-hardening` 权威确认）。
  - ⚠️ 沙箱环境的 gitconfig 锁错误导致 fetch 无法回写本地 remote-tracking ref，本地 `origin/rotation-b1-hardening` 缓存仍显示旧值 `209f0c4`。真机执行 `git fetch origin` 即可刷新，**不影响远端实际状态**（勿手动改 `.git/refs`，2026-08-10 有事故教训）。
- `default_app.asar`（111KB，Electron 发行版自带死文件）RC v2/v3 均存在，app.asar 存在时不加载，不影响发布。
