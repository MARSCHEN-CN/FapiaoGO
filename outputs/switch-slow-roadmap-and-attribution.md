# Preview Switch 优化：路线锁定 + Phase 2 归因前置
...
| 矢量 PDF，313 ms 档 | 313 ms | ~10 ms | **主要是调度 / QoS**（⚠️ 归因未证实，见 §3） |
...
Phase 2  QoS 隔离 → 消除 313 ms 档
   ⚠️ 前置：先做归因（§3），确认目标不是「代理协商」
...
## 3. ⚠️ Phase 2 前置：313 ms 归因（当前最大未知）

把目标锁成「QoS 隔离 → 消除排队档」之前，必须先确认**它真的是排队**。
有三条线索指向另一个可能：
...
| 假设 | 真因 | 解法 | 成本 |
|---|---|---|---|
| **H2** | 代理协商 / PAC 求值 | Electron 加一行启动参数 | **一行配置** |
