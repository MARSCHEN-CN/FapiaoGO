...
- **解析失败 / 发票号为空** → 200 个文件全进失败分支，时间线毫无意义
- **发票号重复** → `importHistoryQuery` 被聚合，网络尾巴会被**假证伪**
...
- 后端 OFD 支持很有限：`file_validator.py:282` 原文写着「**OFD 和 XML 暂时只做基础检查**」
- 当前 `backend/` 根目录已**不存在** `ofd_parser` 模块（只在 `build.bak/server/` 的历史产物里能找到）
- 更关键的是：`xml_parser.py` 提取发票号的标签是 `InvoiceNo / InvoiceCode / Fphm`（数电票 XML 格式），
  **不是** OFD 的 `TextObject/TextCode`。也就是说，造 OFD 时"把号码画成文字"很可能根本提取不到，
  得在 OFD 里内嵌符合标签规范的发票 XML 才行。

所以 S-200-OFD 是个**独立的小课题**：先摸清 OFD 的号码提取路径，再造数据集、再验证。
在此之前硬造一个 OFD 数据集，大概率是"200 份全部解析失败"的坏数据，比没有更糟。

**建议顺序**：先把 S-200 跑完拿到数据 → 我判读归因 → 如果数据显示需要排除 OFD 路径差异，
再单开一轮做 OFD 数据集。
